...
#' @export
sc_metrics_pixels = function(x, raster, dist_fun = "euclidean", compactness, step) {
  ...
  out = sc_metrics_pixels_cpp(clusters, centers_xy, centers_vals, vals,
                              step = step, compactness = compactness,
                              dist_name = dist_name, dist_fun = dist_fun)

  spatial = terra::rast(out[["spatial"]])
  terra::ext(spatial) = terra::ext(raster)
  terra::crs(spatial) = terra::crs(raster)

  value = terra::rast(out[["value"]])
  terra::ext(value) = terra::ext(raster)
  terra::crs(value) = terra::crs(raster)

  combined = terra::rast(out[["combined"]])
  terra::ext(combined) = terra::ext(raster)
  terra::crs(combined) = terra::crs(raster)
