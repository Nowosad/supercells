...
For example, `sc_slic_points()` is helpful for inspecting supercell center locations, while `sc_slic_raster()` is useful for large datasets where polygon outputs may be too memory-intensive.

## Compactness tuning and iteration diagnostics

Use `sc_tune_compactness()` to estimate a compactness value from local value variability at the chosen `step`, then run `sc_slic()` with the tuned value. 
...
`compactness = use_adaptive()` enables adaptive compactness (SLIC0). This lets the method adjust compactness across supercells rather than using a single fixed value.
...
- New utilities: Added `use_meters()` helper for `step` and `use_adaptive()` for adaptive compactness in `sc_slic()`/`sc_slic_points()`/`sc_slic_raster()`.
