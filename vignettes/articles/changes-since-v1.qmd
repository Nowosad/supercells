...
terra::plot(vol)
plot(sf::st_geometry(vol_sc), add = TRUE, lwd = 0.3, border = "red")
```
```{r}
...
terra::plot(vol)
plot(sf::st_geometry(vol_pts), add = TRUE, pch = 16, col = "red", cex = 0.4)
```
```{r}
...
For example, `sc_slic_points()` is helpful for visualizing initial supercell centers or iteration diagnostics, while `sc_slic_raster()` is useful for large datasets where polygon outputs may be too memory-intensive.
...
vol_sc_tuned <- sc_slic(vol, step = 8, compactness = comp_tune$compactness_global)
terra::plot(vol)
plot(sf::st_geometry(vol_sc_tuned), add = TRUE, lwd = 0.3, border = "red")
```

`sc_slic_points(..., iter_diagnostics = TRUE)` attaches iteration
diagnostics so you can visualize convergence in mean distance across iterations later on with `sc_plot_iter_diagnostics()`.
...
Three new functions, `sc_metrics_pixels()`, `sc_metrics_supercells()`, and `sc_metrics_global()`,
provide metrics to evaluate supercell segmentation quality at different levels of detail.
Pixel-level metrics show distances from each cell to its assigned center (lower is more compact/homogeneous). 
Supercell-level metrics summarize those distances by region, and global metrics provide a single-row overview for comparisons.
...
`compactness = "auto"` enables adaptive compactness (SLIC0). This lets the method adjust
compactness across supercells rather than using a single fixed value.
...
terra::plot(vol)
plot(sf::st_geometry(vol_sc_slic0), add = TRUE, lwd = 0.3, border = "blue")
```

## Other changes

- New utilities: Added `step_unit` to `sc_slic()`/`sc_slic_points()`/`sc_slic_raster()` to support map-unit step sizes.
